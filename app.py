...
def admin_menu():
    ...
    markup.add("➕ Добавить админа", "🔙 Главное меню")
...
def get_user_mention(user_id):
    """Показывает только юзернейм (например @username), если нет — его Имя"""
    try:
...
def delete_msg_after_delay(chat_id, message_id, delay=300):
    ...
    try:
        ...
        logging.info(f"Сообщение {message_id} удалено.")
    except Exception as e:
        logging.error(f"Ошибка удаления: {e}")# ========== ОБЩИЕ ХЭНДЛЕРЫ ==========
...
def get_amount(msg):
    ...
    if qr_file_id:
        ...
        sent_msg = bot.send_message(msg.chat.id, f"📱 QR-код временно отсутствует. Поддержка: {SUPPORT}\n\n{instruction_text}")
...
def get_check_photo(msg):
    ...
    if not user_session:
        bot.send_message(msg.chat.id, "❌ Ошибка сессии! Начните заново.")
        start(msg)
        return
    ...
    bot.send_message(msg.chat.id, "✅ Заявка на пополнение отправлена!\n⏳ Ожидайте подтверждения.", reply_markup=main_menu(user_id))# ========== СЦЕНАРИЙ ВЫВОДА ==========
...
def get_withdraw_elqr(msg):
    if msg.text == "🔙 Назад":
        ...
        return
    if not msg.photo:
        bot.send_message(msg.chat.id, "❌ Отправьте фото вашего ELQR кода!", reply_markup=back_menu())
        bot.register_next_step_handler(msg, get_withdraw_elqr)
        return
...
def get_withdraw_code(msg):
    ...
    if not session:
        bot.send_message(user_id, "❌ Сессия истекла.")
        start(msg)
        return
    ...
    for admin in admins:
        try:
            ...
            logging.error(f"Ошибка вывода админу: {e}")
            
    bot.send_message(user_id, "✅ Готово! Заявка обрабатывается.", reply_markup=main_menu(user_id))# ========== АДМИНИСТРИРОВАНИЕ ==========
@bot.message_handler(func=lambda m: m.text == "⚙️ Admin панель" and m.from_user.id in get_admins())
def admin_panel(msg):
    bot.send_message(msg.chat.id, "⚙️ Панель управления", reply_markup=admin_menu())

@bot.message_handler(func=lambda m: m.text == "➕ Добавить админа" and m.from_user.id in get_admins())
def add_admin_btn(msg):
    bot.send_message(msg.chat.id, "👤 Отправьте Telegram ID нового админа цифрами:", reply_markup=back_menu())
...
def process_add_admin(msg):
    ...
    try:
        ...
        bot.send_message(msg.chat.id, f"✅ ID {new_admin_id} добавлен!", reply_markup=admin_menu())
    except ValueError:
        bot.send_message(msg.chat.id, "❌ ID должен состоять только из цифр.", reply_markup=admin_menu())
...
def toggle_bot(msg):
    ...
    if msg.text == "🔴 ВЫКЛЮЧИТЬ БОТА":
        bot_active = False
        bot.send_message(msg.chat.id, "🔴 Бот ВЫКЛЮЧЕН.", reply_markup=admin_menu())
    else:
        bot_active = True
        bot.send_message(msg.chat.id, "🟢 Бот ВКЛЮЧЕН.", reply_markup=admin_menu())

@bot.message_handler(func=lambda m: m.text == "📋 Заявки Пополнение" and m.from_user.id in get_admins())
def view_requests(msg):
    deposits = get_pending_deposits()
    if not deposits:
        bot.send_message(msg.chat.id, "📭 Заявки на пополнение отсутствуют.", reply_markup=admin_menu())
        return
    for dep in deposits:
...
def view_withdraw_requests(msg):
    ...
    if not withdraws:
        bot.send_message(msg.chat.id, "📭 Заявки на вывод отсутствуют.", reply_markup=admin_menu())
        return
    for w in withdraws:
...
def stats(msg):
    ...
    bot.send_message(msg.chat.id, 
        f"📊 СТАТИСТИКА\n\n👥 Пользователей: {users}\n✅ Пополнений: {approved_dep}\n📤 Выводов: {approved_wit}", 
        reply_markup=admin_menu())

@bot.message_handler(func=lambda m: m.text == "🖼 Изменить QR" and m.from_user.id in get_admins())
def change_qr(msg):
    bot.send_message(msg.chat.id, "🖼 Отправьте новый QR-код картинкой:", reply_markup=back_menu())
...
def save_new_qr(msg):
    ...
    if msg.photo:
        ...
        save_qr(file_id)
        bot.send_message(msg.chat.id, "✅ QR-код сохранен!", reply_markup=admin_menu())
    else:
        bot.send_message(msg.chat.id, "❌ Отправьте картинку.", reply_markup=back_menu())
        bot.register_next_step_handler(msg, save_new_qr)# ========== ОБРАБОТКА НАЖАТИЙ КНОПОК АДМИНАМИ ==========
...
def handle_call(call):
    ...
    if action in ["approve", "reject"]:
        ...
        if not res or res[2] != "pending":
            bot.answer_callback_query(call.id, "⚠️ Уже обработано!")
            conn.close()
            return
        ...
        if action == "approve":
            ...
            try: bot.send_message(user_id, f"✅ Заявка на пополнение {amount} сом ОДОБРЕНА!")
            except Exception: pass
            status_text = "✅ ОДОБРЕНО"
        else:
            update_deposit_status(target_id, "rejected")
            try: bot.send_message(user_id, f"❌ Заявка на пополнение {amount} сом ОТКЛОНЕНА! Поддержка: {SUPPORT}")
            except Exception: pass
        ...
        if action == "wapprove":
            ...
            try: bot.send_message(user_id, f"✅ Вывод с {bookmaker} выполнен на ваш ELQR.")
            except Exception: pass
            status_text = "✅ ВЫПЛАЧЕНО"
        else:
            update_withdraw_status(target_id, "rejected")
            try: bot.send_message(user_id, f"❌ Вывод с {bookmaker} отклонен. Поддержка: {SUPPORT}")
            except Exception: pass
            status_text = "❌ ОТКЛОНЕНО"
            
        try: bot.edit_message_caption(f"{status_text} ДЛЯ ВЫВОДА #{target_id}", call.message.chat.id, call.message.message_id, reply_markup=None)
        except Exception: pass
