...
def init_db():
    ...
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS users (chat_id INTEGER PRIMARY KEY, username TEXT, join_date TEXT)''')
...
def add_user(chat_id, username):
    conn = sqlite3.connect('database.db')
    c = conn.cursor()
    c.execute('INSERT OR IGNORE INTO users (chat_id, username, join_date) VALUES (?, ?, ?)', 
              (chat_id, username, datetime.now().strftime("%d.%m.%Y %H:%M")))
    conn.commit()
    conn.close()
...
def start(msg):
    ...
    markup.add(types.InlineKeyboardButton("💬 Наш чат", url="https://t.me/GGKASSAKG"))
    
    bot.send_message(msg.chat.id, 
        f"✨ Добро пожаловать, {user.first_name}!\n\n🏦 GGKASSA - ваш надежный финансовый помощник\n\n👇 Выберите действие:", 
        reply_markup=main_menu(msg.from_user.id))
    
    bot.send_message(msg.chat.id, "📢 Присоединяйтесь к нашему чату для общения!", reply_markup=markup)
...
def support(msg):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("📢 Написать в поддержку", url="https://t.me/helpGGkassabot"))
    bot.send_message(msg.chat.id, f"📞 Свяжитесь с нами:", reply_markup=markup)
...
def get_amount(msg):
    ...
    if qr_file_id:
        ...
        bot.send_message(msg.chat.id, f"📱 QR-код временно отсутствует. Свяжитесь с поддержкой")
...
def withdraw(msg):
    if not bot_active and msg.from_user.id not in get_admins():
        ...
        return
    bot.send_message(msg.chat.id, f"💸 Для вывода средств обратитесь в поддержку:", reply_markup=main_menu(msg.from_user.id))

# ========== АДМИН ПАНЕЛЬ ==========
@bot.message_handler(func=lambda m: m.text == "⚙️ Админ панель" and m.from_user.id in get_admins())
def admin_panel(msg):
    bot.send_message(msg.chat.id, "⚙️ Админ панель GGKASSA", reply_markup=admin_menu())
...
# ========== СТРАНИЦА ДЛЯ ПИНГА (ЧТОБЫ НЕ ЗАСЫПАЛ) ==========
@app.route('/health')
def health():
    return "OK", 200
