...
def get_admins():
    ...
    # Защитная проверка: если главного админа почему-то нет в списке, добавляем его на лету
...
def main_menu(user_id):
    ...
    # ИСПРАВЛЕНО: Текст кнопки изменен строго на русский язык, соответствующий хэндлеру
...
def get_user_mention(user_id):
    """Показывает юзернейм в заявках, если его нет — Имя"""
    try:
...
# ========== ТАЙМЕР АВТОУДАЛЕНИЯ ЧЕКА (5 минут) ==========
...
# ИСПРАВЛЕНО: Текст проверки теперь совпадает с текстом кнопки на 100%
