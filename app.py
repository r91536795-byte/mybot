...
# Ссылка или file_id картинки-подсказки для ввода ID 1xbet
...
def id_input_menu(user_id):
    ...
    return markup# ========== КРИТИЧЕСКИЙ ФИЛЬТР АКТИВНОСТИ ==========
...
def get_withdraw_code(msg):
    ...
    bot.send_message(user_id, "✅ Готово! Средства будут обработаны после подтверждения кода.", reply_markup=main_menu(user_id))
...
def view_requests(msg):
    ...
    for dep in deposits:
        ...
        try: bot.send_photo(msg.chat.id, photo_id, caption=f"🆕 ЗАЯВКА НА ПОПОЛНЕНИЕ #{dep_id}\n👤 Юзер: {user_id}\n💰 {amount} сом\n🆔 Счет: {account_id}\n📅 {date}", reply_markup=markup)
        except Exception: bot.send_message(msg.chat.id, f"🆕 ЗАЯВКА НА ПОПОЛНЕНИЕ #{dep_id}\n👤 Юзер: {user_id}\n💰 {amount} сом\n🆔 Счет: {account_id}\n📅 {date}", reply_markup=markup)
...
def view_withdraw_requests(msg):
    ...
    for w in withdraws:
        ...
        try: bot.send_photo(msg.chat.id, elqr_photo_id, caption=f"📤 ЗАЯВКА НА ВЫВОД #{w_id}\n👤 Юзер: {user_id}\n🎰 БК: {bookmaker}\n🆔 ID в БК: {account_id}\n🔑 Код: {secure_code}\n📅 {date}", reply_markup=markup)
        except Exception: bot.send_message(msg.chat.id, f"📤 ЗАЯВКА НА ВЫВОД #{w_id}\n👤 Юзер: {user_id}\n🎰 БК: {bookmaker}\n🆔 ID в БК: {account_id}\n🔑 Код: {secure_code}\n📅 {date}", reply_markup=markup)
